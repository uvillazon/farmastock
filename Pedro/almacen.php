...
while ($line = mysql_fetch_array($result, MYSQL_ASSOC)) { /*MySQL_ASSOC, es para que use los nombre de los campos y/o                                                           alias que se coloco en la consulta*/
	...
	$id= $line["id_producto"];
	//Bucle 
    foreach ($line as $col_value) {/*se recorre el array dado por $line y en cada iteraci�n, el valor del elemento actual se asigna a                                    $col_value y el puntero interno del array avanza una posici�n  */      
	...
	<img src=\"images/icono_modificar.gif\" width=12 height=12 alt=\"modificar\" title=\"Modificar\" />
	 </a>   </td>";
	
	echo "<td> <a href=\"borrar_producto.php?id_producto=$id\"  onclick=\"return confirmar()\">
...
if (!confirm("¿ Estas seguro de que desea eliminar el Producto ?")) { 
return false; 
} 
else { 
